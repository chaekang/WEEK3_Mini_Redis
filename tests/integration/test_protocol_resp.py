...
STRESS_PATTERNS = ("exact_winner", "exact_ttl_owner")
LONG_TEXT = "x" * 256
...
STRESS_CASES = [
    pytest.param(
        ...
        marks=pytest.mark.xfail(
            strict=False,
            reason=(
                "coarse lock preserves safety but does not guarantee a deterministic "
                "winner ordering for same-key races"
            ),
        ),
        id=f"{pattern}-{mode}-{workers}",
    )
    for pattern, mode, workers in product(
        STRESS_PATTERNS,
        CONCURRENCY_MODES,
        (8, 16),
    )
...
@pytest.mark.parametrize(("pattern", "protocol_mode", "worker_count"), STRESS_CASES)
def test_concurrency_stress_exposes_ordering_limits(
    pattern: str, protocol_mode: str, worker_count: int
) -> None:
    outcome = _run_stress_case(pattern, protocol_mode, worker_count)

    if pattern == "exact_winner":
        assert outcome.final_get == (True, f"value-{worker_count - 1}")
        return
...
def _run_stress_case(
    ...
    key = f"stress:{pattern}:{protocol_mode}:{worker_count}"

    with running_dual_protocol_stack() as harness:
        assert _execute_protocol_command(harness, "resp", "SET", key, "seed") == "OK"

        if pattern == "exact_ttl_owner":
            assert _execute_protocol_command(harness, "resp", "EXPIRE", key, "5") == 1
        ...
        def worker(index: int) -> None:
            ...
            if pattern == "exact_winner":
                sequence = [("SET", (key, f"value-{index}"))]
            else:
                if index % 2 == 0:
                    sequence = [("EXPIRE", (key, "5"))]
                else:
                    sequence = [("PERSIST", (key,))]

            try:
                barrier.wait(timeout=10)
                results = _run_worker_sequence(harness, protocol, sequence)
                with lock:
                    worker_results.append((index, protocol, key, results))
            except BaseException as error:
                with lock:
        ...
        final_get = _execute_protocol_command(harness, "resp", "GET", key)
        final_ttl = _execute_protocol_command(harness, "resp", "TTL", key)
        assert isinstance(final_get, tuple)
        assert isinstance(final_ttl, int)
